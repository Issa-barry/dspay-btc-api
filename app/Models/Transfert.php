...
class WebhookController extends Controller
{
    ...
    protected function finalizeAfterSuccess(PaymentEnLigne $pel): bool
    {
        ...
        DB::transaction(function () use ($pel, &$created) {
            ...
            $modeReception  = $lockedMeta['mode_reception'] ?? Transfert::MODE_ORANGE_MONEY;
            ...
            // Récupération du taux entier
            $taux = (int) ($lockedMeta['taux_applique'] ?? 0);
            if ($taux <= 0) {
                $tauxModel = TauxEchange::find($tauxId);
                $taux = $tauxModel ? (int) $tauxModel->taux : 0;
            }
            if ($taux <= 0) {
            ...
            // Conversion EUR->GNF avec taux entier
    ...
    private function mergePelTwins(PaymentEnLigne $target, PaymentEnLigne $source): void
    {
        ...
        $target->metadata = array_replace($metaS, $metaT);
